...
weather_df |>
  ...
  knitr::kable(digits = 2) # make a table of weather_df
```
...
weather_df |>
  group_by(name, month) |>
  mutate(tmax_rank = min_rank(desc(tmax))) |> # rank days in each month from coldest to hottest, the rank being tmax_rank
  filter(tmax_rank < 2) # select the coldest day of each month
```
...
weather_df |>
  group_by(name) |>
  mutate(yesterday_tmax = lag(tmax)) |> # lag() gets the value 1 row back
  mutate(three_day_before_tmax = lag(tmax, 3)) # go 3 rows back
```
    ...
    ## # ℹ 2,180 more rows
    ## # ℹ 1 more variable: three_day_before_tmax <dbl>
